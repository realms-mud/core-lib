...
{
    return "Display the first lines of a file";
}
...
protected string flagInformation(string flag, string colorConfiguration)
{
    ...
    switch (parsedFlag)
    {
        case "-n":
        {
            ret = "This option will display the first N lines of the "
                "file";
            break;
        }
    }
...
{
    return format("The 'head' command displays the first 5 lines of the "
        ...
        "flag is selected, only the first N lines of the file are displayed.", 
        78);
}
...
{
    return "See also: more, cat, and tail";
}
