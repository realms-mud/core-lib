...
public nomask void registerPort(object port)
{
    ...
    {
        raise_error(sprintf("tradingService.c: Invalid port object (%O).\n", 
            port));
    }
}
...
public nomask object getPort(string portName)
{
    ...
    else
    {
        ...
        raise_error(sprintf("tradingService.c: Port '%s' is not registered.\n", portName));
    }
    return ret;
}
...
public string *getItemListForType(string itemType)
{
    ...
    {
        raise_error(sprintf("tradingService.c: Invalid trading type '%s'.\n", itemType));
    }
    return items;
}
...
public nomask mapping generateDeadline(int minutesFromNow)
{
    ...
    int nowYear = environmentService->currentYear();
    int nowDay = environmentService->currentDay();
    int nowMinute = environmentService->currentTime();
...
public nomask int getMinutesUntilDeadline(mapping deadline)
{
    ...
    {
        raise_error("tradingService.c: Invalid deadline format.\n");
    }
    return ret;
}
