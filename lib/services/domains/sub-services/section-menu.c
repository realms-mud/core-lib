...
//*****************************************************************************
virtual inherit "/lib/services/domains/sub-services/domain-layout.c";

#include "/lib/services/materials/materials.h"

private object MaterialsService = getService("materials");
    ...
    mapping sectionData, mapping materialsToBeUsed)
{
    ...
    object *inventory = filter(deep_inventory(user),
        (: ((member(inherit_list($1), "/lib/items/material.c") > -1) &&
            ...
            MaterialsService->getTypes(type, user));
