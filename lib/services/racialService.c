...
//*****************************************************************************
#include "/lib/services/races/eyes.h"
#include "/lib/services/races/hair.h"
#include "/lib/services/races/races.h"
...
public nomask mapping characterCreationSubraces(string race)
{
    ...
    if (member(races, race) && member(races[race], "subraces"))
    {
        ...
        foreach(string subrace in raceList)
        {
            ...
            object traitObj = TraitService->traitObject(path);

            if(traitObj)
            {
                selection[to_string(i)] = ([
                    "name":capitalize(subrace),
                    "description": TraitService->traitDetailsFromFile(path),
                    "file": path
                ]);
                i++;
            }
